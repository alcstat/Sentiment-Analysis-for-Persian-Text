...
Error=matrix(rep(0,3*100),ncol=3)
...
fig2 <- plot_ly(
  ...
  type = "indicator",
  mode = "gauge+number",
  delta = list(reference = score),
  gauge = list(
    axis =list(range = list(-1, 1)))) 
fig2 <- fig2 %>%
  layout(margin = list(l=20,r=30))
fig2 %>% layout(autosize = F, width = 920, height = 400)
```
...
f$Frequencies=rowSums(Dictionary$Frequencies)

wordcloud2(data=f, size=1.6) 
```
